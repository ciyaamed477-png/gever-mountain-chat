...
export default function ChatPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (s.fontSize === "sm" || s.fontSize === "base" || s.fontSize === "lg") setFontSize(s.fontSize);
    } catch {
      /* */
    }
  }, []);
  ...
  return (
    <div className="flex h-dvh flex-col bg-background text-foreground">
          ...
          {messages.map((m, i) => {
            ...
            return (
              <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                ...
                >
                  {showSenderHeader && (
                    <GroupSenderName senderId={m.sender_id} />
                  )}
      ...
      <form
        ...
        className="flex items-end gap-2 border-t border-border bg-card px-3 py-2 pb-[max(env(safe-area-inset-bottom),8px)]"
      >
        <textarea
          value={text}
          onChange={(e) => {
...
function GroupSenderName({ senderId }: { senderId: string }) {
  ...
  return <div className="mb-0.5 text-xs font-semibold text-primary">{name}</div>;
}
